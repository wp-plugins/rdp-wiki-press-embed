 ...
 */
if ( ! class_exists('rdp_simple_html_dom_node') ) :
/**
...
// dump html dom tree
function rdp_dump_html_tree($node, $show_attr=true, $deep=0)
{
	$node->dump($node);
}
...
class rdp_simple_html_dom
{
	...
	function createElement($name, $value=null) {return @rdp_str_get_html("<$name>$value</$name>")->first_child();}
	function createTextNode($value) {return @end(rdp_str_get_html($value)->nodes);}
	function getElementById($id) {return $this->find("#$id", 0);}
	function getElementsById($id, $idx=null) {return $this->find("#$id", $idx);}
	function getElementByTagName($name) {return $this->find($name, 0);}
